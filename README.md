...
> **写在前面**：这是一套 **AI 初学者也能轻松驾驭** 的工具。它不追求接入更多模型，而是统合你手头已有的模型，让每个模型工作在它最擅长的位置——各就其位，从源头减少 token 浪费。如果你有条件同时开多个 Code Plan，还可以把工具链中的免费成员替换成更强的付费模型：项目完成度更高，后期返工更少。祝各位都能开发出伟大的作品。
...
TUTORIAL.md                     # 详细教程：从零到第一次派单（新手从这里开始）
...
tools/chatparty/                # ChatParty 顾问通道工具链（启动器 + 登录看门狗 + 试跑 + 巡检 + 组长遥控桥 + 主进程补丁工具）
...
默认装到 WorkBuddy 技能目录 `~\.workbuddy\skills\ai-orchestra`（可传参改目标，如 Claude Code 的 `~/.claude/skills/`）。装完即获得**完整功能**：角色制架构、七步流程、任务包人话化模板、通道路由、ChatParty 顾问通道工具链（启动器 + 登录看门狗 + 试跑 + 巡检 + 组长遥控桥 + 主进程补丁工具）、国外顾问组通道工具（foreign-cli：五站三重门发送，页面/iframe 双形态）——并且**自动生成** `capabilities.md` / `routes.yaml`（从脱敏示例复制，不覆盖已有文件），照着 `TUTORIAL.md` 走 15 分钟即可完成第一次派单。

## 快速开始（手动安装）

0. 新手直接读 [TUTORIAL.md](TUTORIAL.md)（15 分钟从零到第一次派单，含三条配置路线）。
...
4. 想用 ChatParty 顾问通道的话，看 `tools/chatparty/README.md`（纯标准库脚本，无第三方依赖）；国外顾问组看 `tools/foreign-cli/README.md`。
